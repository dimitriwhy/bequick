...
def supervised_learning():
    ...
    form_emb, pos_emb, deprel_emb, W0, b0, W1, b1 = model.variable_output(session)
    session.close()
    LOG.info('Session closed.')
    return form_emb, pos_emb, deprel_emb, W0, b0, W1, b1

def reinforcement_learning(form_emb, pos_emb, deprel_emb, W0, b0, W1, b1):
    ...
    model.initialize_from_sl(session, form_emb, pos_emb, deprel_emb, W0, b0, W1, b1)
    logging.info('Embedding and parameters from supervised learning is loaded.')
    memory = Memory(system.num_actions(), opts.memory_size, opts.batch_size_rl)
    ...
    while iteration <= opts.max_iter_rl:
        ...
        if n_batch % (opts.target_update_freq * 20) == 0:
...
def main():
    form_emb, pos_emb, deprel_emb, W0, b0, W1, b1 = supervised_learning()
    reinforcement_learning(form_emb, pos_emb, deprel_emb, W0, b0, W1, b1)
...
if __name__ == "__main__":
    ...
    cmd.add_argument("--eps-decay-steps", dest="eps_decay_steps", type=int, default=200000,
    ...
    cmd.add_argument("--target-update-freq", dest="target_update_freq", type=int, default=100,
