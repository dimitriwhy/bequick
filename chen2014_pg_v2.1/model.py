...
class Classifier(Network):
    def __init__(self, form_size, form_dim, pos_size, pos_dim, deprel_size, deprel_dim, hidden_dim, output_dim,
        ...
        # PREDICTION
        hidden_layer = tf.tanh(tf.add(tf.matmul(inputs, self.W0), self.b0))
        ...
        # self.logprob = tf.log(self.select_prob)
        self.result = tf.multiply(self.select_prob, self.rewards)
        self.entropy = -tf.reduce_sum(tf.multiply(self.prob, tf.nn.softmax(tf.add(tf.matmul(hidden_layer2, self.W1), self.b1))))
        self.pg_loss = tf.reduce_mean(self.result) + l2 * self.entropy
    ...
    def policy_train(self, session, inputs, outputs, rewards):
        ...
        _, cost, indices, prob, res = session.run([self.pg_optimization, self.pg_loss, self.indices, self.prob, self.result],
                              feed_dict={self.form: form, self.pos: pos, self.deprel: deprel,
                                         self.rewards: -rewards, self.indices: indices})
        #print("prob = {0}\nindices = {1}\nresult = {2}".format(prob, indices, res))
        #print(self.count)
        #print("rewords = {0}\noutputs = {1}".format(rewards, outputs))
        self.count += 1
    ...
    def variable_output(self, session):
        form_emb, pos_emb, deprel_emb, W0, b0, W1, b1 = session.run([self.form_emb, self.pos_emb, self.deprel_emb,
...
class DeepQNetwork(Network):
    def __init__(self, form_size, form_dim, pos_size, pos_dim, deprel_size, deprel_dim, hidden_dim, output_dim, dropout,
        ...
        with tf.device("/cpu:0"), tf.name_scope("embedding"):
            ...
            tgt_inputs = tf.concat(1, [
                ...
                tf.reshape(tf.nn.ebedding_lookup(self.tgt_pos_emb, self.pos), [-1, self.pos_features_dim]),
    ...
    def initialize_from_sl(self, session, form_emb, pos_emb, deprel_emb, W0, b0, W1, b1):
        session.run([self.form_emb.assign(form_emb),
                     self.pos_emb.assign(pos_emb),
                     self.deprel_emb.assign(deprel_emb),
                     self.W0.assign(W0),
                     self.b0.assign(b0),
                     self.W1.assign(W1),
                     self.b1.assign(b1)])
