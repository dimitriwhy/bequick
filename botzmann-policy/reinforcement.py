...
def main():
    ...
    cmd.add_argument("--eps-init", dest="eps_init", type=float, default=100., help="The initial value of eps.")
    cmd.add_argument("--eps-final", dest="eps_final", type=float, default=0.01, help="The final value of eps.")
    ...
    while iteration <= opts.max_iter:
        ...
        while not s.terminate():
            ...
            prediction = model.target_policy(session, x)[0]
            sum_value = 0
            max_value = np.max(prediction).item()
            for id in range(prediction.size) :
                if valid_mask[id] :
                    prediction[id] = math.exp((prediction[id].item() - max_value) / eps)
                    sum_value += prediction[id].item()
            p = np.random.rand()
            prep_p = 0
            chosen_id = -1
            # LOG.info("valid_ids={3}, valid_mask={0}, prediction={1}, p={2}".format(valid_mask, prediction, p, valid_ids))
            for id in range(prediction.size) :
                if valid_mask[id] :
                    prep_p += prediction[id] / sum_value
                    chosen_id = id
                    if prep_p >= p :
