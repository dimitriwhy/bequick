...
class DeepQNetwork(Network):
    def __init__(self, form_size, form_dim, pos_size, pos_dim, deprel_size, deprel_dim, hidden_dim, output_dim, dropout,
        ...
        self.loss = tf.reduce_mean(tf.multiply(self.weights,tf.square(predicted_q - self.output))) + l2 * regularizer
    ...
    def train(self, session, inputs, action, outputs, weights):
        form, pos, deprel = inputs
        _, cost = session.run([self.optimization, self.loss], feed_dict={
                              self.form: form, self.pos: pos, self.deprel: deprel,
                              self.action: action, self.output: outputs, self.weights: weights})
        return cost
