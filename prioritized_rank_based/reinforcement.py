...
class Memory(object):
    def __init__(self, n_actions, memory_size, batch_size, steps):
        ...
        self.experience = Experience({'size': memory_size, 'batch_size': batch_size, 'steps':steps})
    ...
    def add(self, s_t_form, s_t_pos, s_t_deprel, a_t, r_t, s_t_plus_1_form, s_t_plus_1_pos, s_t_plus_1_deprel,
        ...
        self.experience.store((self.current_id,0,0,0,0))
    ...
    def sample(self, global_steps):
        #ids = np.random.choice(self.volume(), self.batch_size)
        ids, weights, _upd_ids = self.experience.sample(global_steps)
        #print(ids)
        ...
        return (self.s_t_form[ids], self.s_t_pos[ids], self.s_t_deprel[ids],
                ...
                self.s_t_plus_1_valid_mask[ids], self.terminate[ids], np.array(weights), _upd_ids)

    def update(self, ids, td_error) :
        self.experience.update_priority(ids, list(td_error))
...
def main():
    ...
    memory = Memory(system.num_actions(), opts.memory_size, opts.batch_size, opts.prioritized_flourish_steps)
