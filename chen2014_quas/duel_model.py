...
class DeepQNetwork(Network):
    def __init__(self, form_size, form_dim, pos_size, pos_dim, deprel_size, deprel_dim, hidden_dim, output_dim, dropout,
        ...
        self.mle_loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(
            tf.add(tf.matmul(hidden_layer2, self.W1), self.b1), self.action)) + l2 * regularizer
        self.mle_optimization = tf.train.AdagradOptimizer(learning_rate=0.1).minimize(self.mle_loss)

        actions_one_hot = tf.one_hot(self.action, self.output_dim, 1.0, 0.0, name='action_one_hot')
        predicted_q = tf.reduce_sum((tf.nn.tanh(tf.add(tf.matmul(hidden_layer2, self.W1), self.b1))+1)/2.0 * actions_one_hot,
                                    reduction_indices=1)
        ...
        self.optimization = tf.train.AdamOptimizer(learning_rate=lr).minimize(self.loss)
    ...
    def oracle_train(self, session, inputs, outputs):
        form, pos, deprel = inputs
        _, cost = session.run([self.mle_optimization, self.mle_loss],
                              feed_dict={self.form: form, self.pos: pos, self.deprel: deprel, self.action: outputs})
        return cost
