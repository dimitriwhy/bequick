...
try:
    ...
    from .duel_model import Classifier, DeepQNetwork, initialize_word_embeddings
...
def las_calculate(rewards):
    ...
    p = np.random.rand()
    if p < 0.005:
        LOG.info("Sample sampling LAS: {0}".format(precision))
    return precision
...
def main():
    ...
    cmd.add_argument("--eps-init", dest="eps_init", type=float,
                     default=.2, help="The initial value of eps.")
    cmd.add_argument("--eps-final", dest="eps_final", type=float,
                     default=0.01, help="The final value of eps.")
    ...
    cmd.add_argument("--memory-size", dest="memory_size", type=int,
                     default=500000, help="The size of memory")
    ...
    #    model = Classifier(form_size=len(form_alphabet), form_dim=100,
    #                       pos_size=len(pos_alphabet), pos_dim=20,
    #                       deprel_size=len(deprel_alphabet), deprel_dim=20,
    #                       hidden_dim=opts.hidden_size,
    #                       output_dim=system.num_actions(),
    ...
    # n_batch, n_samples = 0, oracle_Ys.shape[0]
    # best_uas = 0.
    ...
    #         xs, ys = (oracle_forms[batch_id], oracle_postags[batch_id], oracle_deprels[batch_id]), oracle_Ys[batch_id]
    ...
    sl_count = 0
    n_samples = oracle_Ys.shape[0]
    LOG.info("oracle n_samples = {0}".format(n_samples))
    order = np.arange(n_samples)
    ...
    while iteration <= opts.max_iter:
        if n_batch % len(train_dataset) == 0:
            cost_oracle, cost_explore = 0., 0.
        ...
        if pn < las/(best_las + (1e-6)):
            ...
            if sl_count == 10:
                ...
                batch_id = order[batch_start: batch_end]
                xs, ys = (oracle_forms[batch_id], oracle_postags[batch_id], oracle_deprels[batch_id]), oracle_Ys[batch_id]
                cost_oracle += model.oracle_train(session, xs, ys)
        ...
        if (opts.evaluate_stops > 0 and n % opts.evaluate_stops == 0)\
           or n_batch % len(train_dataset) == 0:
            uas, las = evaluate(devel_dataset, session, parser, model,
                                devel_feats, True, opts.lang)
            if n_batch % len(train_dataset) == 0:
                LOG.info('Iteration {0} done, eps={1}, \
Cost(oracle+explore)={2}+{5}, UAS={3}, LAS={4}'
                         .format(iteration, eps, cost_oracle, uas, las, cost_explore))
                iteration += 1
                pn -= 1
            else:
