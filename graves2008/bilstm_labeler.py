...
LOG = logging.getLogger('graves2008/bilstm-labeler')
...
class Model(object):
    def __init__(self, algorithm, form_size, form_dim, hidden_dim, output_dim, n_layers, max_steps, batch_size,
        ...
        logits = tf.reshape(tf.add(tf.matmul(output, self.W1), self.b1), (self.batch_size, -1, self.output_dim))
    ...
    def classify(self, session, x, steps):
        return session.run(self.pred, feed_dict={self.x: x, self.steps: steps})
...
def evaluate(dataset, session, model):
    ...
    for n in range(0, l, batch_size):
        ...
        y_pred = model.classify(session, x, step)
        y_pred = np.argmax(y_pred, axis=2)
        n_total += len(y[y > 1])
        n_corr += np.array(y[y > 1] == y_pred[y > 1], dtype=np.int32).sum()
...
def main():
    ...
    cmd.add_argument("--max-iter", dest="max_iter", type=int, default=30, help="The number of max iteration.")
