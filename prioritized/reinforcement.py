...
class Memory(object):
    def __init__(self, n_actions, memory_size, batch_size, alpha, beta, steps):
        ...
        # Based on Schaul et al. 2016, alpha = 0.6, beta_0 = 0.4 for the proportional variant.
        self.experience = Experience(memory_size, batch_size, self.alpha)
    
    def add(self, s_t_form, s_t_pos, s_t_deprel, a_t, r_t, s_t_plus_1_form, s_t_plus_1_pos, s_t_plus_1_deprel,
            s_t_valid_mask, terminate, td_error):
        if td_error == None :
            td_error = self.experience.get_max_val()
            if td_error == 0:
                td_error = 1
        ...
        self.experience.add(self.current_id, td_error)
    ...
    def sample(self):
        ...
        ids, weights, _ = self.experience.select(self.beta)
        ...
        return (self.s_t_form[ids], self.s_t_pos[ids], self.s_t_deprel[ids],
                ...
                self.s_t_plus_1_valid_mask[ids], self.terminate[ids], np.array(weights), _ids)

    def update(self, ids, td_error) :
        self.experience.priority_update(ids, td_error)
...
def main():
    ...
    cmd.add_argument("--prioritized-alpha", dest="prioritized_alpha", type=float, default = 0.6,
                     help="alpha factor for prioritized expereince replay")
    cmd.add_argument("--prioritized-beta", dest="prioritized_beta", type=float, default = 0.4,
                     help="alpha factor for prioritized expereince replay")
    cmd.add_argument("--prioritized-flourish-steps", dest="prioritized_flourish_steps", type=int, default=1000000,
                     help="the number of steps for beta from the init to increase to the final")
    opts = cmd.parse_args()
    ...
    memory = Memory(system.num_actions(), opts.memory_size, opts.batch_size, opts.prioritized_alpha,
                    opts.prioritized_beta, opts.prioritized_flourish_steps)
    ...
    while memory.volume() < opts.replay_start_size:
        ...
        while not s.terminate():
            ...
            memory.add(x[0], x[1], x[2], chosen_id, r, next_x[0], next_x[1], next_x[2], valid_mask, s.terminate(), None)
    ...
    while iteration <= opts.max_iter:
        ...
        while not s.terminate():
            ...
            _ys = np.amax(model.policy(session, xs), axis = 1)

            td_error = abs(ys - _ys)
            cost += model.train(session, xs, actions, ys, weights)
            #print(np.multiply(td_error,weights))
            memory.update(ids, td_error)
