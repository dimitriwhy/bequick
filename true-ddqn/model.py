...
class DeepQNetwork(Network):
    def __init__(self, form_size, form_dim, pos_size, pos_dim, deprel_size, deprel_dim, hidden_dim, output_dim, dropout,
        ...
        with tf.device("/cpu:0"), tf.name_scope("embedding"):
            ...
            self.tmp_form_emb = tf.Variable(random_uniform_matrix(self.form_size, self.form_dim), name = "tmp_form_emb")
            self.tmp_pos_emb = tf.Variable(random_uniform_matrix(self.pos_size, self.pos_dim), name = "tmp_pos_emb")
            self.tmp_deprel_emb = tf.Variable(random_uniform_matrix(self.deprel_size, self.deprel_dim), name="tmp_deprel_emb")
        ...
        self.tmp_W0 = tf.Variable(random_uniform_matrix(self.input_dim, self.hidden_dim), name="tmp_W0")
        self.tmp_b0 = tf.Variable(tf.zeros([self.hidden_dim]), name="tmp_b0")
        self.tmp_W1 = tf.Variable(random_uniform_matrix(self.hidden_dim, self.output_dim), name="tmp_W1")
        self.tmp_b1 = tf.Variable(tf.zeros([self.output_dim]), name="tmp_b1")
        
        # PARAM SYNC

        self.tgt2tmp_op = [
            self.tmp_form_emb.assign(self.tgt_form_emb),
            self.tmp_pos_emb.assign(self.tgt_pos_emb),
            self.tmp_deprel_emb.assign(self.tgt_deprel_emb),
            self.tmp_W0.assign(self.tgt_W0),
            self.tmp_b0.assign(self.tgt_b0),
            self.tmp_W1.assign(self.tgt_W1),
            self.tmp_b1.assign(self.tgt_b1)
        ]
        ...
        self.tmp2q_op = [
            self.form_emb.assign(self.tmp_form_emb),
            self.pos_emb.assign(self.tmp_pos_emb),
            self.deprel_emb.assign(self.tmp_deprel_emb),
            self.W0.assign(self.tmp_W0),
            self.b0.assign(self.tmp_b0),
            self.W1.assign(self.tmp_W1),
            self.b1.assign(self.tmp_b1)
        ]
    ...
    def exchange(self, session):
        session.run(self.tgt2tmp_op)
        session.run(self.update_op)
        session.run(self.tmp2q_op)
