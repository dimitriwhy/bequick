    ...
    while iteration <= opts.max_iter:
        ...
        while not s.terminate():
            ...
            if p > eps:
                ...
                tgt_prediction = model.target_policy(session, x)[0]
                prediction = (prediction + tgt_prediction) / 2
            ...
            next_ys = model.policy(session, next_xs)
            next_ys[~next_valid_mask] = -1e30
            chosen_ids = next_ys.argmax(axis = 1)
            tgt_next_ys = model.target_policy(session, next_xs)
            ys_max = np.zeros(len(tgt_next_ys))
            for i in range(len(tgt_next_ys)) :
                ys_max[i] = tgt_next_ys[i][chosen_ids[i]]
            ys += (1 - terminated) * ys_max * opts.discount
