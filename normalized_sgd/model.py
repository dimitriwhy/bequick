...
class DeepQNetwork(Network):
    def __init__(self, form_size, form_dim, pos_size, pos_dim, deprel_size, deprel_dim, hidden_dim, output_dim, dropout,
        ...
        #NOMALIZED PARAMS
        self.sigma = tf.Variable(1.0, name = "sigma")
        self.mu = tf.Variable(0.0, name = "mu")
        self.upsilon = tf.Variable(0.1, name = "upsilon")
        self.beta = tf.Variable(0.0001, name = "beta")
        self.one = tf.Variable(1.0, name = "one")
        ...
        # NOMALIZED PARAM UPD
        self.normalized_update_op = [
            self.upsilon.assign((self.one - self.beta) * self.upsilon + self.beta * tf.reduce_mean(tf.square(self.output))),
            self.mu.assign((self.one - self.beta) * self.mu + self.beta * tf.reduce_mean(self.output)),
            self.sigma.assign(self.upsilon - self.mu * self.mu),
            # self.beta.assign(self.one / (self.one / self.beta + self.one))
        ]
        ...
        #NOMALIZED Q_FUNCTION
        self.nomalized_q_function = tf.sqrt(self.sigma) * self.q_function + self.mu;
        ...
        self.optimizer = tf.train.GradientDescentOptimizer(learning_rate=0.00025)

        norm_gradients = self.optimizer.compute_gradients(self.loss, var_list = [self.W0, self.b0])
        norm_gradients = [(tf.reciprocal(self.sigma) * g, v) for g, v in norm_gradients]
        gradients = self.optimizer.compute_gradients(self.loss, var_list = [self.W1, self.b1])
        gradients.extend(norm_gradients)
        self.apply_gradients = self.optimizer.apply_gradients(gradients)
    ...
    def update_normalized_parms(self, session, outputs):
        session.run(self.normalized_update_op, feed_dict = {self.output: outputs})


    def norm_train(self, session, inputs, action, outputs) :
        form, pos, deprel = inputs
        
        self.update_normalized_parms(session, outputs)
        
        _, cost = session.run([self.apply_gradients, self.loss], feed_dict = {
            self.form: form, self.pos: pos, self.deprel: deprel, self.action: action, self.output: outputs})
        return cost
    ...
    def get_norm_parms(self, session) :
        return session.run([self.beta, self.upsilon, self.mu, self.sigma])
