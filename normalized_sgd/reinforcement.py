...
def main():
    ...
    while iteration <= opts.max_iter:
        ...
        while not s.terminate():
            ...
            cost += model.norm_train(session, xs, actions, ys)
            beta, upsilon, mu, sigma = model.get_norm_parms(session)
            # LOG.info('beta={0}, upsilon={1}, mu={2}, sigma={3}, actions={4}'.format(beta, upsilon, mu, sigma, np.mean(ys)))
            # LOG.info('cost={0}'.format(cost))
            eps -= eps_decay_rate
