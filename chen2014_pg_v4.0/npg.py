...
if __name__ == "__main__":
    ...
    cmd.add_argument("--dropout", type=float, default=-1, help="The probability for dropout.")
    cmd.add_argument("--language", dest="lang", default="en", help="the language")
    #Argument for reinforcement learning
    cmd.add_argument("--batch-size-rl", dest="batch_size_rl", type=int, default=512, help="The size of batch for rl.")
    ...
    cmd.add_argument("--learning-rate-rl", dest="learning_rate_rl", type=float, default=0.0001, help="Learning rate for RL.")
    cmd.add_argument("--sampling-freq", dest="samp_freq", type=int, default=8, help="Sampling frequency.")
    ...
    model = Classifier(form_size=len(form_alphabet), form_dim=100, pos_size=len(pos_alphabet), pos_dim=20,
                       ...
                       learning_rate_rl=opts.learning_rate_rl)
    ...
    while iteration <= opts.max_iter_rl:
        if n % len(train_dataset) == 0:
            cost = 0
            logging.info("Start of policy gradient iteration {0}, data shuffled.".format(iteration))
            np.random.shuffle(train_dataset)
        data = train_dataset[n % len(train_dataset)]

        for sample_t in range(opts.samp_freq):
            s = State(data)
            ...
            for i in range(len(rewards)):
                rewards[i] = score - length / 2
            total_rewards += rewards
        
        if n % opts.batch_size_rl == 0 :                
            ...
            cost += model.policy_train(session, (forms, postags, deprels), Ys, rewards)
            total_forms, total_postags, total_deprels, total_Ys, total_rewards = [], [], [], [], []
